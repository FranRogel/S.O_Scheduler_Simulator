...
class PriorityScheduler(Scheduler):
    ...
    def next_process_high_priority(self, current_process):
        # Selección del proceso con mayor prioridad
        self.ready_queue.sort(key=lambda p: p.priority)
        process = self.ready_queue[0]
        return process.priority < current_process.priority
    ...
    def run(self):
        ...
        while self.processes or self.ready_queue or self.waiting_queue or process_running:
            ...
            elif process_running and self.ready_queue and self.next_process_high_priority(current_process):
                event = f"Time {self.current_time}: Process {current_process.name}: lost the processor because a process with higher priority appeared."
                print(event)  # Imprimir el evento antes de agregarlo
                events.append(event)
                process_running = False 
                taken_cpu = False

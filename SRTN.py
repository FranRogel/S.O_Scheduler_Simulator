...
class SRTN(Scheduler):
    ...
    def next_process_short_burst_remaining(self, current_process):
        process = self.ready_queue[0]
        return process.burst_remaining < current_process.burst_remaining
        ...
        while self.processes or self.ready_queue or self.waiting_queue or process_running:
            ...
            # 2. Verificar si el proceso en running tiene que bloquearse (De Running a Bloqueado)
                ...
                if switch_time == self.tcp:
                    ...
                    current_process.burst_remaining = current_process.burst_duration
            ...
            elif process_running and self.ready_queue and self.next_process_short_burst_remaining(current_process):
                event = f"Time {self.current_time}: Process {current_process.name}: lost the processor because a process with lower burst appeared."
                print(event)  # Imprimir el evento antes de agregarlo
                events.append(event)
                process_running = False 
                taken_cpu = False
                current_process.burst_remaining = burst_time
                self.add_to_ready_queue(current_process)
                pass
            ...
            if process_running and init_time == self.tip and not(taken_cpu):
                burst_time = current_process.burst_remaining  # Establecer la ráfaga actual del proceso
                taken_cpu = True
